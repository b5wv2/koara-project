...
code = code.replace(/if \(!response\.ok\) {/g, 'if (!response.ok || data.success === false) {');
...
if (code.match(oldInputDiv)) {
  ...
  .replace('onChange={(e) => setEmail(e.target.value)}', 'onChange={(e) => { setEmail(e.target.value); setErrorMsg(\\'\\'); }}')
  + `\n            {errorMsg && (
