...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const isSmallGPU = navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 4;
    const isLowDPR = window.devicePixelRatio < 1.5;

    let effectiveQuality = quality;
    if (isMobile) effectiveQuality = 'very-low';
    else if (isTablet || isSmallGPU || isLowDPR) effectiveQuality = 'low';
    else effectiveQuality = 'medium';

    const qualitySettings = {
      'very-low': { iterations: 8, waveIterations: 1, pixelRatio: Math.min(window.devicePixelRatio, 0.5), precision: 'lowp', stepMultiplier: 3.0, useNoise: false },
      'low': { iterations: 12, waveIterations: 1, pixelRatio: Math.min(window.devicePixelRatio, 0.75), precision: 'lowp', stepMultiplier: 2.0, useNoise: false },
      'medium': { iterations: 24, waveIterations: 1, pixelRatio: Math.min(window.devicePixelRatio, 1.0), precision: 'mediump', stepMultiplier: 1.5, useNoise: true },
      'high': {
        iterations: 40,
        waveIterations: 2,
    ...
    const fragmentShader = `
      ...
      const bool USE_NOISE = ${settings.useNoise ? 'true' : 'false'};
      ...
      void main() {
        ...
        if (USE_NOISE && uNoiseIntensity > 0.0) {
    ...
    let targetFPS = 60;
    if (effectiveQuality === 'very-low') targetFPS = 24;
    else if (effectiveQuality === 'low') targetFPS = 30;
