        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          <div className="mt-auto flex items-center justify-between gap-2 mb-3">
            {product.selling_price ? (
              <span className="sf-display font-black text-lg" style={{ color: custom.primaryColor || '#FFFFFF' }}>{formatCurrency(parseFloat(product.selling_price), store?.store_currency)}</span>
            ) : product.sale_price !== null && product.sale_price !== undefined ? (
              <div className="flex flex-col">
                <span className="text-xs line-through mb-0.5" style={{ color: '#475569' }}>{formatCurrency(parseFloat(product.price), store?.store_currency)}</span>
                <span className="sf-display font-black text-lg" style={{ color: custom.primaryColor || '#FFFFFF' }}>{formatCurrency(parseFloat(product.sale_price), store?.store_currency)}</span>
              </div>
            ) : (
              <span className="sf-display font-black text-lg" style={{ color: custom.primaryColor || '#FFFFFF' }}>{formatCurrency(parseFloat(product.price), store?.store_currency)}</span>
            )}
            ...
            <form onSubmit={handleSubmitOrder} className="space-y-5">
                    ...
                    {cartItems.map((item) => (
                      <div key={item.cartItemId} className="flex justify-between items-center p-2.5 rounded-xl bg-white/5 border border-white/10 text-xs">
                        ...
                        <div className="font-mono text-blue-400 font-bold shrink-0" dir="ltr">
                          {item.quantity}x {formatCurrency(item.selling_price, store?.store_currency)}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                selectedProduct && (
                  <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    ...
                    <div className="min-w-0">
                      ...
                      <span className="text-xs font-mono text-blue-400" dir="ltr">{formatCurrency(calculateTotal(), store?.store_currency)}</span>
                    </div>
                  </div>
                )
              )}
              ...
              <div className="rounded-xl p-4 space-y-2" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                <div className="flex justify-between items-center">
                  ...
                  <span className="sf-display text-xl font-black" style={{ color: '#3B82F6' }} dir="ltr">{formatCurrency(calculateTotal(), store?.store_currency)}</span>
                </div>
              </div>
...
const Storefront = ({ store }) => (
  <CartProvider storeId={store?.id} storeCurrency={store?.store_currency}>
