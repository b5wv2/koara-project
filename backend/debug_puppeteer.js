  ...
  try {
    ...
    const unifiedOrdersCTE = `
      WITH unified_orders AS (
        ...
        FROM topup_orders WHERE store_id = $1 AND created_at >= $2
      )
    `;
    ...
    const recentOrdersRes = await pool.query(`
      ...
      FROM unified_orders ORDER BY created_at DESC LIMIT 10
    `, [storeId, store.starts_at]);
    ...
    console.log(`[PUPPETEER_DIAGNOSIS] HTML saved to report-debug.html. Length: ${htmlContent.length} chars`);
    
    // Parse some basic stats from the HTML
    ...
    // 4. Wrap these individually
    console.log("[PUPPETEER_DIAGNOSIS] Calling page.setContent()...");
    try {
    ...
    console.log("[PUPPETEER_DIAGNOSIS] Calling page.emulateMediaType('screen')...");
    try {
      await page.emulateMediaType('screen');
      console.log("[PUPPETEER_DIAGNOSIS] page.emulateMediaType() SUCCESS");
    } catch (e) {
      console.error("[PUPPETEER_DIAGNOSIS] page.emulateMediaType() FAILED:", e.message);
      throw e;
    }

    console.log("[PUPPETEER_DIAGNOSIS] Calling page.pdf()...");
    try {
      const pdfBuffer = await page.pdf({
        format: 'A4',
        printBackground: true
      });
      console.log(`[PUPPETEER_DIAGNOSIS] page.pdf() SUCCESS! Generated buffer length: ${pdfBuffer.length}`);
    } catch (e) {
      console.error("[PUPPETEER_DIAGNOSIS] page.pdf() FAILED:", e.message);
      throw e;
    }

    console.log("Full diagnosis completed successfully.");
  } catch (err) {
    console.error("Diagnosis encountered an error:", err);
  } finally {
    if (browser) {
      await browser.close();
    }
